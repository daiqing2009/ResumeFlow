RESUME_CONVERT = """
Convert the following resume data in plain text into JSON format, including keys as 'name', 'phone', 'email', 'linkedin', 'summary', 'highlights', 'experience', 'education', 'interests', 'skills', 'additional'.
Note that just keep the value of keys as empty if no relevant info is found in the plain text.
"""
JD_CONVERT = """
Convert the following job description in plain text into JSON format, including keys as 'company_name', 'url', 'job_title', 'responsibilities', 'qualifications', 'preferred_experience'.
Note that just keep the value of keys as empty if no relevant info is found in the job description.
"""
